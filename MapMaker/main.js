...
var canvas;
var img;
var wid = prompt("Set the width of the map:");
var hei = prompt("Set the height of the map:");
...
function setup() {
  ...
  collideables = createCheckbox("Collideable");
  grass = createCheckbox("Grass");
  water = createCheckbox("Water");
  warpTiles = createCheckbox("Warp Tile");
  but.mouseReleased(function () {
    var mapName = inp.value();
    server.name = mapName;
    saveJSON(layers, mapName);
    saveJSON(server, "server_" + mapName);
  });
...
function setTile() {
  ...
  var tiq = { x: cx * 32, y: cy * 32 };
  if (collideables.checked()) server.collideables.push(tiq);
  else if (grass.checked()) server.grass.push(tiq);
  else if (water.checked()) server.water.push(tiq);
  else if (warpTiles.checked()) server.warpTiles.push(tiq);
}
function draw() {
